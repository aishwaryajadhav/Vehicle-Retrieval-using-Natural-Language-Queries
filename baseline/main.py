...
best_top1_eval = 0.
def evaluate(model,valloader,epoch,cfg,index=2,eval=False):
    ...
    if (not eval) and (top1_acc.avg > best_top1_eval):
...
parser.add_argument('--name', default="baseline", type=str,
                    help='experiments')
parser.add_argument('--evaluate', action='store_true', help='evaluation only or not')
...
valloader = DataLoader(dataset=val_data, batch_size=cfg.EVAL.BATCH_SIZE, shuffle=False, num_workers=cfg.TRAIN.NUM_WORKERS)
